  ...
  return (
    <header>
      <div className="flex lg:h-[5rem] xxs:h-[4rem] items-center justify-between text-sm w-full lg:border-none xxs:border-b xxs:border-gray-500 p-4">
        ...
        {/* wrapped the icon with search bar  */}
        <div className="xxs:flex xxs:justify-between xxs:items-center xxs:gap-5">
          {/* logo  */}
          <img
            src={logo}
            alt="logo"
            className="w-6 h-10 xxs:flex  lg:hidden "
          />

          {/* search bar for mobile screen  */}
          <div className="xxs:h-[2.1rem] xxs:relative  xxs:top-[0.2px]">
            <label className="relative flex items-center xxs:mb-5 xxs:w-40 xxs:flex lg:hidden w-full">
              {/* Input field*/}
              <input
                className={`placeholder:italic placeholder:text-slate-500 text-slate-300 bg-transparent outline-none w-full border border-slate-500 rounded-full py-2 pl-2 pr-3 
              ${isSearchVisible ? 'block' : 'hidden'} 
              lg:block
              `}
                placeholder="Search for News"
              ...
              <span
                onClick={toggleSearchVisibility}
                className="absolute right-3 text-slate-500 h-full content-center cursor-pointer w-7 "
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 -960 960 960"
