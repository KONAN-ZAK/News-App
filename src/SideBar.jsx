...
function SideBar({ onCategorySelect, onHomeClick }) {
  ...
  return (
    ...
    >
      <div
        className="hover:bg-gray-600 transition ease-in-out duration-500 opacity-60 flex self-end p-2 w-fit h-fit rounded-full 
         cursor-pointer m-2 lg:hidden"
      ...
      <div className="h-full w-full flex flex-col gap-8">
        <div className="flex justify-center items-center gap-1 w-full h-36 xxs:flex-col md:flex-row">
          ...
          <h1 className="xxs:text-[1rem] md:text-[1.8rem] font-extrabold">
            News
          </h1>
        </div>
        ...
        {sideBarData.map((sideData) => (
          ...
          *: hover:border-mainColor hover:ring-mainColor hover:ring-2
          ...
          onClick={() => {
            onCategorySelect(sideData.title);
          }}>
            {sideData.title}
          </div>
        ))}
